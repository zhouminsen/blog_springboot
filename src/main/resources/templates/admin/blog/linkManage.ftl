...
<script type="text/javascript">
	...
	function deleteLink(){
		...
		if(selectedRows.length==0){
		 ...
		 $.messager.confirm("系统提示","您确定要删除这<font color=red>"+selectedRows.length+"</font>条数据吗？",function(r){
				if(r){
					$.post("${cxt}/admin/link/delete",{ids:ids},function(result){
						if(result.success){
	...
	function openLinkModifyDialog(){
		var selectedRows=$("#dg").datagrid("getSelections");
		 ...
		 $("#fm").form("load",row);
		 url="${cxt}/admin/link/modify?id="+row.id;
	 }
...
<table id="dg" title="友情友情链接管理" class="easyui-datagrid"
   fitColumns="true" pagination="true" rownumbers="true"
   url="${cxt}/admin/link/list" fit="true" toolbar="#tb"
   pageList="[1,2,3,5,10,20,50]" pageSize="5">
   <thead>
   	<tr>
   		...
   		<!-- sortable 启动排序-->
   		<th field="orderNo" width="100" align="center" sortable="true">排序序号</th>
   	</tr>
   </thead>
 </table>
 <div id="tb">
