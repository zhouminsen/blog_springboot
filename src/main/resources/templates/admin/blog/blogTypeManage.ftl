...
$(function(){
	...
	$("#dg").datagrid({
		url:"${cxt}/admin/blog/type/list?v="+Math.random(),
		//数据表格的数据
		title:"博客类别管理",
		...
		toolbar:[
		   ...
		   {text:"删除",iconCls:"icon-remove",handler:function(){
		  	  if (flag) {
				   ...
				   $.post("${cxt}/admin/blog/type/delete?v="+Math.random(),{"ids":ids},function(result){
					   if (result) {
		   ...
		   {text:"查询",iconCls:"icon-search",handler:function(){
	        	 ...
	        	 $("#searchBtn").click(function(){
		        	 ...
		        	 $("#dg").datagrid("load",{"startDate":startDate,"endDate":endDate,"typeName":typeName,"deleteFlag":deleteFlag});
	        	 });
		...
		onAfterEdit:function(index,row){
			//直接把数据更新到服务器
			if(method=="save"){
				$.post("${cxt}/admin/blog/type/save?v="+Math.random(),row,function(result){
				...
				$.ajax({
					url:"${cxt}/admin/blog/type/modify?v="+Math.random(),
					data:row,
					dataType:"json",
					type:"post",
		...
		columns:[[
		     ...
		     {field:"orderNo",title:"排列序号",width:100,align:"center",sortable:true,
		    	 editor:{
		    		 type:"numberbox",
		    		 options:{
		    			 required:true,
		    			 missingMessage:"请输入正确的序号",
		    			 min:0
		    		 }
		    	 }
		     },
		     ...
		     {field:"deleteFlag",title:"删除标记",width:100,align:"center",
		      ...
		      {field:"blogCount",title:"博文数量",width:100,align:"center",formatter:function(val,row,index){
		    	 if(row.blogCount>=10){
		    		 return "<span style='color:green'><B>"+val+"</b></span>";
		    	 }else if(val>=80){
		    		 return "<span style='color:lime'><B>"+val+"</b></span>";
		    	 }else if(val>=60){
		    		 return "<span style='color:blue'><B>"+row.blogCount+"</b></span>";
		    	 }else{
		    		 return "<span style='color:red'><B>"+row.blogCount+"</b></span>";
		    	 }
		     }} 
		  ]],
...
<body style="margin: 1px">
 ...
 <div class="easyui-panel" title="查询数据" collapsible="true" collapsed="true" id="searchDiv">
		<table>
			<Tr>
				<td>类型名称</td><td><input type="text" id="typeName" name="typeName"/></td>
				<td>创建日期</td><td><input type="text" id="startDate" name="startDate" class="easyui-datebox"/>至</td>
