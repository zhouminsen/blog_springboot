...
<script type="text/javascript">
	...
	function submitData(){
		...
		if(content==null || content==''){
			...
			$.post("${cxt}/comment/save",{'content':content,'imageCode':imageCode,'blogId':'${blog.id}'},function(result){
				if(result.success){
		...
		<c:choose>
			...
			<c:otherwise>
				<#list commentList as comment>
						<c:choose>
							<c:when test="${comment_index<10 }">
								<div class="comment">
									<span><font>${comment_index+1 }楼&nbsp;&nbsp;&nbsp;&nbsp;${comment.userIp }：</font>${comment.content }&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;[&nbsp;<fmt:formatDate value="${comment.commentDate }" type="date" pattern="yyyy-MM-dd HH:mm"/>&nbsp;]</span>
								</div>								
							</c:when>
							<c:otherwise>
