...
import org.zjw.blog.util.FormatStyle;
...
public class FileUtil {
    ...
    */
	public static String getFileSuffix(String fileName) {
		...
		if (fileName.lastIndexOf('.') > 0) {
			...
			String suffix = fileName.substring(start, strLength);
	...
	/**
	 *  目录下已经有同名文件,则文件重命名,增加文件序号 add by tony 20110712 否则创建新的文件
	 * @param filePath 父目录
	 * @param fileName 文件名称
	 * @return
	 * @throws IOException 
	 */
	public static synchronized File createNewFile(String filePath, String fileName) throws IOException{
		String newFileName = new String();
		String withoutExt = new String();
		File curFile = new File(filePath + "\\" + fileName);
		if (curFile.exists()) {
			for(int counter = 1; curFile.exists(); counter++){
                ...
                newFileName = withoutExt + "(" + counter + ")" + "." + getFileSuffix(curFile.getName());
                curFile = new File(filePath + "\\" + newFileName);
               
            }
		} else {
			//当前文件的父目录不存在,则创建父目录然后再创建该文件
			if (!curFile.getParentFile().exists()) {
				//mkdirs()会一直判断该目录的父目录,如果有则创建
   ...
   */
  public static synchronized void clearDir(String dirPath)
   throws FileNotFoundException {
   ...
   * @throws IOException
   */
  public static synchronized void deleteDir(String dirPath)
   ...
   * @throws IOException
   */
  public static synchronized void deleteDir(File file)
  throws FileNotFoundException,IOException {
  ...
  public static synchronized void rename(String oldName,String newName)
   throws FileNotFoundException {
   	boolean b = false;
	File d = new File(oldName);
	if(d.exists()){
		b = d.renameTo(new File(newName));
	}
  }
  ...
  /**
   * 读日志文件 "c:\\Log.txt"
   * <br>输入参数：sFile = Path + FileName 文件路径＋文件名称
   */
 ...
 */
  ...
  public ArrayList CreateXML(String StartIndent,String indent,String[] aTrunk,String[] aLeaf,String[] aContent) {
	...
	// 边生成边写XML文件
	public String WriteXML(String sFile, String sXmlVer, String root,
			ArrayList aList) {
		String sMsg = "";
		long lTime = System.currentTimeMillis();
		try {
			...
			sMsg = this.getClass().getName() + " WriteXML IOException Error."
					+ ex.getMessage();
		} catch (Exception ex) {
			sMsg = this.getClass().getName() + " WriteXML Exception Error."
					+ ex.getMessage();
		}
		return sMsg;
	}
	 ...
	 * @throws IOException */
	public void copyFileIgnore(String file1, String file2) throws IOException {
